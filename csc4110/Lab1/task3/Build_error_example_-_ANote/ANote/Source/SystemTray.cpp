...
void CSystemTray::OnTimer(UINT nIDEvent) 
{
	...
	if (nIDEvent == ID_TRAYSINGLECLICK)	{
	
		KillTimer (ID_TRAYSINGLECLICK);

	// Send the message to mainframe that the user clicked with the left mouse-button
		CWnd *pTargetWnd = GetTargetWnd();
		pTargetWnd->SetForegroundWindow ();
		pTargetWnd->SendMessage (WM_COMMAND, ID_PRESSEDLEFTBUTTONONTRAYICON, 0);
		m_bSingleClicked = FALSE;
	}
...
// Dow we have an alarm?
	CMainFrame *pMain = (CMainFrame*) ::AfxGetMainWnd ();
	if (pMain->ShowAlarmDialog ())
		return 0L;
...
// Add any minimized notes here
	BCMenu *pMinimized = NULL;
	pMinimized = (BCMenu *)pSubMenu->GetSubMenu (6);
	pMain->CreateMinimizedMenu ((BCMenu *)pSubMenu, pMinimized);

	BCMenu *a, *b;
	a = (BCMenu *) pSubMenu->GetSubMenu (1);
	b = (BCMenu *) pSubMenu->GetSubMenu (2);
// Change the text to the right language
	menu.SetMenuText (ID_TRAYMAIN_NEWNOTE,						CTranslate::ReadString (0, "New Note"),					MF_STRING);

	a->SetMenuText (ID_TRAYMAIN_IMPORTEXPORT_IMPORT,			CTranslate::ReadString (2, "Import"),					MF_STRING);
	a->SetMenuText (ID_TRAYMAIN_IMPORTEXPORT_EXPORT,			CTranslate::ReadString (3, "Export"),					MF_STRING);
	b->SetMenuText (ID_TRAYMAIN_UPLOADDOWNLOAD_UPLOADNOTES,		CTranslate::ReadString (5, "Upload"),					MF_STRING);
	b->SetMenuText (ID_TRAYMAIN_UPLOADDOWNLOAD_DOWNLOADNOTES,	CTranslate::ReadString (6, "Download"),					MF_STRING);
	menu.SetMenuText (ID_TRAYMAIN_STAYONTOP,					CTranslate::ReadString (7, "Stay on top"),				MF_STRING);
	menu.SetMenuText (ID_TRAYMAIN_MINIMIZEALL,					CTranslate::ReadString (8, "Minimize all"),				MF_STRING);
	menu.SetMenuText (ID_TRAYMAIN_RECYCLEBIN,					CTranslate::ReadString (10, "Recycle bin..."),			MF_STRING);
	menu.SetMenuText (ID_TRAYMAIN_VISITANOTEWEBSITE,			CTranslate::ReadString (11, "Visit A Note's website"),	MF_STRING);
	menu.SetMenuText (ID_TRAYMAIN_CHECKFORUPDATES,				CTranslate::ReadString (12, "Check for updates"),		MF_STRING);
	menu.SetMenuText (ID_TRAYMAIN_FAQ,							CTranslate::ReadString (13, "FAQ"),						MF_STRING);
	menu.SetMenuText (ID_TRAYMAIN_SENDBUGG,						CTranslate::ReadString (14, "Send Bug / Feedback..."), MF_STRING);
	menu.SetMenuText (ID_TRAYMAIN_OPTIONS,						CTranslate::ReadString (15, "Options"),					MF_STRING);
	menu.SetMenuText (ID_APP_ABOUT,								CTranslate::ReadString (16, "About"),					MF_STRING);
	menu.SetMenuText (ID_APP_EXIT,								CTranslate::ReadString (17, "Exit"),					MF_STRING);
...
// Do we have an alarm?
	...
	// Make sure that the programm don't think we have singleclicked
		KillTimer (ID_TRAYSINGLECLICK);
		m_bSingleClicked = FALSE;
        ...
        if (m_DefaultMenuItemByPos)
        {
            ...
            pSubMenu = (BCMenu *) menu.GetSubMenu(0);
            if (!pSubMenu)
                return 0;
	...
	// We have singleclicked. To be realy sure that the user didn't double-click
	// we have to start a timer.
        SetTimer(ID_TRAYSINGLECLICK, GetDoubleClickTime (), NULL);		
